...
class ForecastIO{
  ...
  function getCurrentConditions($latitude, $longitude) {
    ...
    if ($data !== false) {
      ...
      $jsonConditions = new ForecastIOConditions($data->currently);
      return json_encode((array)$jsonConditions);
   ...
   * @return \ForecastIOConditions|boolean
   */
  function getAllConditionData($latitude, $longitude) {

    $data = $this->requestData($latitude, $longitude);

    if ($data !== false) {

      $jsonConditions = new ForecastIOConditions($data);
      return json_encode((array)$jsonConditions);
